            ...
            <div class="container md:px-5 mx-auto">
              ...
              <div class="lg:w-2/3 w-full mx-auto overflow-auto">
                <div class="flex justify-end mb-4">
                  <button onclick="location.href='{{ route('family.image.create') }}'" class="text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">新規投稿</button>
                </div>
                  ...
                  <tbody>
                    @foreach($images as $image)
                      <tr>
                        ...
                        <td>
                          <button onclick="location.href='{{ route('family.image.edit', ['image'=> $image->id]) }}'" class="text-white bg-gray-300 border-0 py-2 md:px-4 focus:outline-none hover:bg-indigo-600 rounded">編集</button>
                        </td>
                        <form id="delete_{{ $image->id }}" action="{{ route('family.image.destroy', ['image' => $image->id]) }}" method="post">
                          @csrf
                          @method('delete')
                          <td>
                            <a href="#" onclick="deletePost(this)" data-id="{{ $image->id }}" class="text-white bg-gray-300 border-0 py-2 md:px-4 focus:outline-none hover:bg-red-600 rounded">削除</a>
                          </td>
                        </form>
                      </tr>
                    @endforeach
