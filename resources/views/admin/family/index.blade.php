                ...
                <table class="table-auto w-full text-left whitespace-no-wrap">
                  ...
                  <tbody>
                    @foreach($families as $family)
                      <tr>
                        <td class="md:px-4 py-3">{{ $family->name }}</td>
                        <td class="md:px-4 py-3">{{ $family->email }}</td>
                        <td class="md:px-4 py-3">{{ $family->created_at->diffForHumans() }}</td>
                        <td>
                          <button onclick="location.href='{{ route('admin.family.edit', ['family'=> $family->id]) }}'" class="text-white bg-gray-300 border-0 py-2 md:px-4 focus:outline-none hover:bg-indigo-600 rounded">編集</button>
                        </td>
                        <form id="delete_{{ $family->id }}" action="{{ route('admin.family.destroy', ['family' => $family->id]) }}" method="post">
                          @csrf
                          @method('delete')
                          <td>
                            <a href="#" onclick="deletePost(this)" data-id="{{ $family->id }}" class="text-white bg-gray-300 border-0 py-2 md:px-4 focus:outline-none hover:bg-red-600 rounded">削除</a>
                          </td>
                        </form>
                      </tr>
                    @endforeach
